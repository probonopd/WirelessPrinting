...
from . import WirelessPrintOutputDevice
...
##      This plugin handles the connection detection & creation of output device objects for WirelessPrint-connected printers.
#       Zero-Conf is used to detect printers, which are saved in a dict.
#       If we discover an instance that has the same key as the active machine instance a connection is made.
@signalemitter
class WirelessPrintOutputDevicePlugin(OutputDevicePlugin):
    def __init__(self):
        super().__init__()
        ...
        self._preferences.addPreference("wirelessprint/manual_instances", "{}")

        try:
            self._manual_instances = json.loads(self._preferences.getValue("wirelessprint/manual_instances"))
        except ValueError:
            self._manual_instances = {}
        if not isinstance(self._manual_instances, dict):
            self._manual_instances = {}

        self._name_regex = re.compile("WirelessPrint instance (\".*\"\.|on )(.*)\.")
    ...
    def startDiscovery(self):
        ...
        self._browser = ServiceBrowser(self._zero_conf, u'_wirelessprint._tcp.local.', [self._onServiceChanged])
    ...
    def addManualInstance(self, name, address, port, path, useHttps = False):
        ...
        self._preferences.setValue("wirelessprint/manual_instances", json.dumps(self._manual_instances))
...
#                    self._instances[key].close()
    ...
    def addInstance(self, name, address, port, properties):
        instance = WirelessPrintOutputDevice.WirelessPrintOutputDevice(name, address, port, properties)
        self._instances[instance.getKey()] = instance
        global_container_stack = Application.getInstance().getGlobalContainerStack()
        if global_container_stack and instance.getKey() == global_container_stack.getMetaDataEntry("wirelessprint_id"):
