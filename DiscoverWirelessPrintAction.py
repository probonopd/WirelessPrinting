...
class DiscoverWirelessPrintAction(MachineAction):
    def __init__(self, parent = None):
        super().__init__("DiscoverWirelessPrintAction", catalog.i18nc("@action", "Connect WirelessPrint"))

        self._qml_url = "DiscoverWirelessPrintAction.qml"
        self._window = None
        self._context = None
    ...
    def startDiscovery(self):
        if not self._network_plugin:
            self._network_plugin = Application.getInstance().getOutputDeviceManager().getOutputDevicePlugin("WirelessPrinting")
    ...
    def setKey(self, key):
        ...
        if global_container_stack:
            if "wirelessprint_id" in global_container_stack.getMetaData():
                global_container_stack.setMetaDataEntry("wirelessprint_id", key)
            else:
                global_container_stack.addMetaDataEntry("wirelessprint_id", key)
    ...
    def getStoredKey(self):
        ...
        if global_container_stack:
            ...
            if "wirelessprint_id" in meta_data:
                return global_container_stack.getMetaDataEntry("wirelessprint_id")
    ...
    def _createAdditionalComponentsView(self):
        Logger.log("d", "Creating additional ui components for WirelessPrint-connected printers.")

        path = QUrl.fromLocalFile(os.path.join(PluginRegistry.getInstance().getPluginPath("WirelessPrinting"), "WirelessPrintComponents.qml"))
        ...
        if not self._additional_components_view:
            Logger.log("w", "Could not create additional components for WirelessPrint-connected printers.")
            return

        Application.getInstance().addAdditionalComponent("monitorButtons", self._additional_components_view.findChild(QObject, "openWirelessPrintButton"))
